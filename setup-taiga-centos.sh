...
MEDIA_URL = "http://10.110.45.244/media/"
STATIC_URL = "http://10.110.45.244/static/"
ADMIN_MEDIA_PREFIX = "http://10.110.45.244/static/admin/"
SITES["front"]["scheme"] = "http"
SITES["front"]["domain"] = "10.110.45.244"
...
git checkout stable
cd dist/
sed -e 's/localhost/10.110.45.244/' conf.example.json > conf.json
...
server {
    ...
    location /api {
        ...
        proxy_pass http://10.110.45.244:8001/api;
        proxy_redirect off;
    }
    ...
    location /admin {
        ...
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_pass http://10.110.45.244:8001$request_uri;
        proxy_redirect off;
    }
